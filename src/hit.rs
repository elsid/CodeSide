    ...
    for i in 0 .. number_of_directions {
        ...
        if let Some(hit) = get_nearest_hit(unit_id, src, dst, target, world) {
            ...
            if let Some(explosion) = explosion {
                let mut explosions = vec![(explosion, source + Vec2::only_x(hit.distance).rotated(angle), is_teammate)];
                ...
                while let Some((explosion, center, is_teammate)) = explosions.pop() {
                    ...
                    for unit in world.units().iter() {
                        ...
                        if *health_mut > 0 && explosion_rect.has_collision(&unit.rect()) {
                            ...
                            if is_teammate {
                                teammate_units_damage_from_teammate += damage * world.is_teammate_unit(unit) as i32;
                                opponent_units_damage_from_teammate += damage * !world.is_teammate_unit(unit) as i32;
                                target_damage_from_teammate += damage * (unit.id == target.id) as i32;
                            } else {
                                teammate_units_damage_from_opponent += damage * world.is_teammate_unit(unit) as i32;
                                opponent_units_damage_from_opponent += damage * !world.is_teammate_unit(unit) as i32;
                                target_damage_from_opponent += damage * (unit.id == target.id) as i32;
                            }
